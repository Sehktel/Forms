//console.log("Фурнитура (по изделиям)"); СДЕЛАНО ИЗ ИТОГОВОЙ
debugger;
...
function draw_row_name(row_product, table_div) {
    ...
    <table class="border" style="width: 100%" cellpadding="2" cellspacing="2">
        <tbody>
            <tr>
                <td rowspan="3"></td>
                <td id="svg" rowspan="3" style="width: 30%; height: 180px; text-align:center">#svg#</td>
                <td style="width:15%"> Номенклатура </td>
                <td colspan="3" >#Номенклатура#</td>
            </tr>
            <tr>
                <td style="width:15%"> Характеристика </td>
                <td colspan="3" >#Характеристика#</td>
            </tr>
            <tr>
                <td style="width:15%"> Размеры </td>
    ...
    table_row.innerHTML = `
                <td style="width:08%">№Ств.</td>
                <td colspan="2" style="width:30%">Наименование</td>
                <td style="width:15%">Высота руч.</td>
                <td style="width:17%">Напр.открывания</td>
                <td style="width:30%">Размер по фальцу</td>`;
    table_body.firstElementChild.appendChild(table_row);

    row_product.characteristic.constructions.forEach((ff) => {
        if (ff.furn != $p.utils.blank.guid) {
            table_row = document.createElement("tr");
            table_row.innerHTML = `
                <td style="width:08%">#constrn#</td>
                <td colspan="2" style="width:30%">#name#</td>
                <td style="width:15%">#handle_h#</td>
                <td style="width:17%">#direct#</td>
                <td style="width:30%">#falce#</td>`;
            table_line = {
                constrn: ff.cnstr - 1,
                name: ff.furn.name,
                handle_h: ff.h_ruch,
                direct: ff.direction,
                falce: ff.w.round(0) + " x " + ff.h.round(0) + " мм.",
            }
            table_row.innerHTML = dhx4.template(table_row.innerHTML, table_line);
            table_body.firstElementChild.appendChild(table_row);
        }
...
return obj.print_data().then((res) => {
    ...
    .then((print_data) => {
        ...
        p.forEach((row) => {
            if (row.characteristic.hasOwnProperty('_attachments') && row.characteristic._attachments.hasOwnProperty('svg')) {
                var cc = [];
                global_price = 0;
                ...
                draw_table(tpl_grouping, nom_sort[i], sort_noms['Фурнитура']);
