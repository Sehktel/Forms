...
console.log("4.2 Приложение №2 5");
debugger;
...
return obj.print_data().then((res) => {
    ...
    if (print_data.Изделия.length) {
        ...
        for (let i = 0; i < print_data.Изделия.length; i++) {
            ...
            //prod_data.ЦенаСоСкидкой = print_data.Цена * (100 - print_data.СкидкаПроцент);
            ...
            total_products_obj.ИтогоСумма += prod_data.Сумма;
            total_products_obj.ВсегоПозиций = print_data.Изделия.length;
            total_products_obj.СредняяСкидка += prod_data.СкидкаПроцент;
            total_products_obj.ИтогоЦенаБезСкидок += prod_data.Цена * prod_data.Количество;
            ...
            prod_data.Сумма = add_zero(prod_data.Сумма.toLocaleString('ru-RU'));
        ...
        total_products_obj.СредняяСкидка = (total_products_obj.СредняяСкидка / total_products_obj.ВсегоПозиций).round(2);
        ...
        total_products_obj.СредняяСкидка = add_zero(total_products_obj.СредняяСкидка.toLocaleString('ru-RU'));
        //Сформируем строку Итого
        //let template_total_products = document.createElement("div");
        //template_total_products.innerHTML = templates.total_products.innerHTML;

        //template_total_products.innerHTML = dhx4.template(template_total_products.innerHTML, total_products_obj);
    ...
    if (print_data.ДопПродукция.length) {
        ...
        for (let i = 0; i < print_data.ДопПродукция.length; i++) {
            ...
            total_products_obj.ИтогоЦена += prod_data.Цена;
            total_products_obj.ИтогоСумма += prod_data.Сумма;
            total_products_obj.ВсегоПозиций = print_data.ДопПродукция.length;
            total_products_obj.СредняяСкидка += prod_data.СкидкаПроцент;
            total_products_obj.ИтогоЦенаБезСкидок += prod_data.Цена * prod_data.Количество;
    ...
    if (print_data.Услуги.length) {
        ...
        for (let i = 0; i < print_data.Услуги.length; i++) {
            ...
            total_products_obj.ИтогоЦена += prod_data.Цена;
            total_products_obj.ИтогоСумма += prod_data.Сумма;
            total_products_obj.ВсегоПозиций = print_data.Услуги.length;
            total_products_obj.СредняяСкидка += prod_data.СкидкаПроцент;
            total_products_obj.ИтогоЦенаБезСкидок += prod_data.Цена * prod_data.Количество;
