...
return obj.print_data().then((res) => {
    ...
    for (var i = 0; i < print_data.Продукция.length; i++) {
        ...
        if (production[prod_data.НомерСтроки - 1].ordn == $p.utils.blank.guid) { //"00000000-0000-0000-0000-000000000000"
            ...
            console.log(production[prod_data.НомерСтроки - 1]);
            cc = $p.cat.characteristics.get(production[prod_data.НомерСтроки - 1].characteristic);
            console.log(cc);
            furName = furParam = "";
            cc.params.each((row) => {
